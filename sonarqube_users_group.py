...
def sonarqube_user_operate_func(sonarhandler, username, gitlab_user_info, sonarqube_users_info):
    """
    处理用户数据
    :param sonarhandler:
    :param username:
    :param gitlab_user_info:
    :param sonarqube_users_info:
    :return:
    """
    ...
    if username not in sonarhandler.users:
        ...
        # 更新用户数据
        if name != sonarqube_users_info[username]['name'] or email != sonarqube_users_info[username]['email']:
...
if __name__ == '__main__':
    ...
    for i in range(len(gitlab_usernames)):
        pool.apply_async(sonarqube_user_operate_func, args=(sonarhandler, gitlab_usernames[i], gitlab_users_info[gitlab_usernames[i]], sonarqube_users_info,))
    pool.close()
    pool.join()
