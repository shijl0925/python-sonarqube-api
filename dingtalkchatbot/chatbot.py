...
class DingtalkChatbot(object):
    ...
    def send_text(self, msg, is_at_all=False, at_mobiles=None):
        ...
        if is_not_null_and_blank_str(msg):
            ...
            print(u"text类型，消息内容不能为空！")
            raise ValueError(u"text类型，消息内容不能为空！")

        if at_mobiles:
            at_mobiles = list(map(str, at_mobiles))

        data["at"] = {"atMobiles": at_mobiles, "isAtAll": is_at_all}
        print(u'text类型：%s' % data)
        self.post(data)
    ...
    def send_link(self, title, text, message_url, pic_url=''):
        ...
        if is_not_null_and_blank_str(title) and is_not_null_and_blank_str(text) and is_not_null_and_blank_str(message_url):
            ...
            print(u'link类型：%s' % data)
            self.post(data)
        else:
            print(u"link类型中消息标题或内容或链接不能为空！")
            raise ValueError(u"link类型中消息标题或内容或链接不能为空！")
    ...
    def send_markdown(self, title, text, is_at_all=False, at_mobiles=None):
        ...
        :param is_at_all: @所有人时：true，否则为：false
        :param at_mobiles: 被@人的手机号（在text内容里要有@手机号）,列表类型
        :return: 返回消息发送结果
        """
        if is_not_null_and_blank_str(title) and is_not_null_and_blank_str(text):
            ...
            print(u"markdown类型：%s" % data)
            self.post(data)
        else:
            print(u"markdown类型中消息标题或内容不能为空！")
            raise ValueError(u"markdown类型中消息标题或内容不能为空！")
    ...
    def send_action_card(self, action_card):
        ...
        if isinstance(action_card, ActionCard):
            ...
            print(u"ActionCard类型：%s" % data)
            self.post(data)
        else:
            print(u"ActionCard类型：传入的实例类型不正确！")
            raise TypeError(u"ActionCard类型：传入的实例类型不正确！")
    ...
    def post(self, data):
        ...
        try:
            ...
            print(u"消息发送失败， HTTP error: %d, reason: %s" % (exc.response.status_code, exc.response.reason))
            raise
        except requests.exceptions.ConnectionError:
            print(u"消息发送失败，HTTP connection error!")
            raise
        else:
            result = response.json()
            print(u'发送结果：%s' % result)
            if result['errcode']:
                error_data = {"msgtype": "text", "text": {"content": u"钉钉机器人消息发送失败，原因：%s" % result['errmsg']}, "at": {"isAtAll": True}}
                print(u"消息发送失败，重新发送：%s" % error_data)
...
class ActionCard(object):
    ...
    def get_data(self):
        ...
        if is_not_null_and_blank_str(self.title) and is_not_null_and_blank_str(self.text) and len(self.btns):
            ...
            print(u"ActionCard类型，消息标题或内容或按钮数量不能为空！")
            raise ValueError(u"ActionCard类型，消息标题或内容或按钮数量不能为空！")
...
class FeedLink(object):
    ...
    def get_data(self):
        ...
        if is_not_null_and_blank_str(self.title) and is_not_null_and_blank_str(self.message_url) and is_not_null_and_blank_str(self.pic_url):
            ...
            print(u"FeedCard类型单条消息文本、消息链接、图片链接不能为空！")
            raise ValueError(u"FeedCard类型单条消息文本、消息链接、图片链接不能为空！")
